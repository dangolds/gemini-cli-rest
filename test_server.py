...
from server import clean_response
...
# ---------------------------------------------------------------------------
# Unit tests for clean_response() — no server required
# ---------------------------------------------------------------------------

class TestCleanResponse:
    """Unit tests for the clean_response() parser."""

    def test_single_line_response(self):
        """Single-line Model: response is extracted correctly (regression)."""
        raw = (
            "User:  Hello\n"
            "Model:  Hi there!\n"
            "❯ Type your message\n"
        )
        result = clean_response(raw, "Hello")
        assert result == "Hi there!"

    def test_multiline_code_block(self):
        """Multi-line code block after Model: prefix is fully captured."""
        raw = (
            "User:  Write a greeting function\n"
            "Model:  Here is some code:\n"
            "1 function greet(name: string) {\n"
            '2   return `Hello, ${name}!`;\n'
            "3 }\n"
            "❯ Type your message\n"
        )
        result = clean_response(raw, "Write a greeting function")
        assert "Here is some code:" in result
        assert "function greet" in result
        assert "Hello, ${name}!" in result
        assert "}" in result
        # Should NOT include TUI chrome
        assert "❯" not in result

    def test_multiple_redraws_picks_longest_block(self):
        """When TUI redraws produce multiple Model: blocks, the longest wins."""
        raw = (
            "Model:  He\n"
            "❯ responding\n"
            "Model:  Hello, world!\n"
        ...
        result = clean_response(raw, "Say hello")
        assert result == "Hello, world!"

    def test_multiline_with_multiple_redraws(self):
        """Longest block wins even when blocks have continuations."""
        raw = (
            "Model:  Here is code:\n"
            "1 print('hi')\n"
            "❯ responding\n"
        ...
        result = clean_response(raw, "Write python code")
        assert "def greet(name):" in result
        assert 'print(greet(' in result

    def test_tui_chrome_excluded(self):
        """TUI chrome lines are not included in the response."""
        raw = (
        ...
        result = clean_response(raw, "What is the answer?")
        assert result == "The answer is 42."
        assert "Shortcuts" not in result
        assert "YOLO" not in result

    def test_fallback_when_no_model_prefix(self):
        """Fallback path works when output has no Model: prefix."""
        raw = (
            "User:  hello\n"
            "❯ prompt\n"
            "The answer is 42.\n"
    ...
    def test_code_block_with_blank_lines_preserved(self):
        """Blank lines inside a code block are preserved (not treated as chrome)."""
        raw = (
        ...
        result = clean_response(raw, "write code")
        assert "Here is the code:" in result
        assert "def foo():" in result
        assert "pass" in result
        assert "YOLO" not in result

    def test_code_block_with_blank_line_separator(self):
        """A blank line between Model: text and code doesn't break the block."""
        raw = (
            "Model:  Here is some code:\n"
            "\n"
            "1 function greet(name: string) {\n"
    ...
    def test_chrome_interspersed_between_model_and_code(self):
        """Real PTY format: chrome lines appear between Model: and code lines."""
        raw = (
            "YOLO mode   Type your message or @path/to/fileModel:  Here is some code:\n"
            "/app no sandbox (see /docs) /model Auto (Gemini 3)\n"
            "1 function greet(name: string): string {\n"
            '2   return `Hello, ${name}!`;\n'
            "3 }\n"
            "4 const result = greet(\"world\");\n"
            "5 console.log(result);\n"
            "responding Move to the start of the line (esc to cancel, 6s)  ? for shortcuts \n"
            "End of code.\n"
            "/app no sandbox (see /docs) /model Auto (Gemini 3)\n"
            " ? for shortcuts \n"
        ...
        result = clean_response(raw, "Repeat back the following text")
        assert "Here is some code:" in result
        assert "function greet" in result
        assert "Hello, ${name}!" in result
        assert "console.log(result)" in result
        assert "End of code." in result
        # Chrome must NOT leak into response
        assert "/app no sandbox" not in result
        assert "YOLO" not in result
