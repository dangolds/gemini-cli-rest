...
| `AGY_MODEL` | `Gemini 3.8 Flash (High)` | Passed as `--model` to every session. Reasoning effort is part of the model name in agy (`agy models` lists Low and High as separate entries), so this is how effort is set. Its own variable, not `AGY_EXTRA_ARGS`: the name contains spaces. Wins over the `model` in `settings.json`, which agy rewrites itself |
